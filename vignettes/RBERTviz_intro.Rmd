...
```{r, eval=FALSE}
attn2 <- format_attention(feats_chicken$attention_probs, seq_num = 2)
visualize_attention(attn2)
```

Again, the attention weights reflect the correct
...
feats_chicken2 <- RBERT::extract_features(examples = chicken_ex2,
                                         ...
                                         layer_indexes = 1:12,
                                         batch_size = 2L)
attn <- format_attention(feats_chicken2$attention_probs, seq_num = 2)
...
train_embeddings_df <- extract_vectors_df(feats_train$layer_outputs)
