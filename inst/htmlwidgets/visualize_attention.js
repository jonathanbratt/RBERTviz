  ...
  factory: function(el, width, height) {
    ...
    return {
      ...
      renderValue: function(x) {
            // x is some json containing appropriately formatted attention data.

            // Code below was taken (with minimal alterations) from:
            ...
            function renderText(svg, text, is_top, att_data, left_pos) {
              // att_data: list of tuples (att, att_trans), one for each layer.
              // att and att_trans are attention matrics for each layer.
            ...
            function draw_checkboxes(config, top, svg, attention_heads) {
              ...
              // Edited below by JDB to add "Head:" label.
              var headText = checkboxContainer.append("text")
                                          .text("Head: ")
                                          .style("cursor", "default")
                                          .style("-webkit-user-select", "none")
                                          .style("text-anchor", "bottom")
                                          .attr("x", CHECKBOX_SIZE)
                                          .attr("y", top + CHECKBOX_SIZE * 0.8);
              // end edit
              var checkbox = checkboxContainer.selectAll("rect")
                                              ...
                                              .attr("x", function(d, i) {
                                                return (i+1+2.3) * CHECKBOX_SIZE; // edited by JDB
                                              })
                                              .attr("y", top)
            ...
            function render() {
              ...
              //renderVis(el, top_text, bot_text, attention, config);
            }

            // Generalizing to any number of layers. -JDB
            var num_layers = attention['all']['att'].length;
